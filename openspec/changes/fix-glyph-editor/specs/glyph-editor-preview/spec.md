...
The system SHALL render the Glyph node map and node states using Compose Canvas as a software preview, independent of the physical Glyph hardware. The renderer SHALL use the same node layout instance as the gesture system, ensuring hit-testing and rendering share identical node positions.
...
#### Scenario: Gesture and renderer share layout
- **WHEN** user taps on a rendered node
- **THEN** the gesture system identifies the same node (layout is shared, not duplicated)
